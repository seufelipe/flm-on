...
// The bare list of suggested double bills — reused by ComboSuggestions (mobile, with its own
// card shell) and PlanPanel (desktop rail / mobile sheet, inside that surface's shell).
export function ComboList({ combos, keyOf, onSelect }: Props) {
  return (
      ...
      {combos.map((combo) => {
        ...
        return (
          <li
            ...
            className="border-4 border-border rounded-btn bg-surface shadow-btn-secondary p-4 flex flex-wrap items-center gap-x-3 gap-y-2 cursor-pointer transition-transform active:translate-x-[3px] active:translate-y-[3px] active:shadow-none"
          >
...
export default function ComboSuggestions(props: Props) {
  return (
