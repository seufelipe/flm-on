...
CLAUDE.md carries only the three the app reads at build time; this is the full inventory.

**Committed, read at build time** — the app's own inputs:
...
**Committed, applied at fetch time** — baked into `staging-batch.json` by `aggregate`, so an edit
to any of these needs a **re-fetch**, not a reload:
...
| `director-overrides.json` | normalized title | `lib/directorOverrides.ts` |

**Gitignored**, regenerated by the weekly scripts: `cache.json` (6h TTL, `lib/cache.ts`),
`letterboxd-cache.json` (no TTL) and `staging-batch.json` (the unpublished week).

Copy a key from the report rather than typing one — the normalized keys keep the cinema's own
typography, curly apostrophe included, so a straight `'` silently adds a second dead entry.
Full symptom → file → key mapping: `SKILL.md`, Step 5.
