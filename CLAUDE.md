...
- `components/FilmLabel.tsx` — a curated editorial tag (from `data/film-labels.json`, see
  decision #11) rendered before a film's title as text on a full-circle SVG `textPath`,
  spinning slowly via the `flm-label-spin` keyframe in `app/globals.css` (the project's only
  runtime SVG + only CSS animation). Decorative — `--color-fg`/`--color-border`, never accent.
...
11. **Curated per-film editorial labels (`data/film-labels.json`).** Added 2026-08-28. A
    ...
    `components/FilmLabel.tsx` as spinning circular text before the title — decorative, so
    per decision #7 it uses `--color-fg`/`--color-border`, never `--color-accent`, and per
    decision #8 it must not become a count/badge. Keyed on title alone (not `Title|Year`
...
- `film-labels.json` — `Record<"<normalized title>", string>`, curated editorial tags shown
  spinning before the film title (decision #11). Render-time only; not in `showtimes.json`.
